...
import com.json_xml.parse.models.dto.partCarSale.input.PartRootDto;
...
import static com.json_xml.parse.constants.Paths.PART_XML_FILEPATH;
...
public class PartServiceImpl implements PartService {
    ...
    public PartServiceImpl(PartRepository partRepository, ModelMapper modelMapper, IOUtil ioUtil, ValidationUtil validation, Gson gson, XmlParser xmlParser, SupplierService supplierService) {
    ...
    public void seedData() throws IOException, JAXBException {
        ...
        ioUtil.print("Data will seed from " + PART_XML_FILEPATH + "\nPLEASE WAIT...");
        String content = String.join("", ioUtil.readFile(PART_XML_FILEPATH));
        xmlParser.importXMl(PartRootDto.class, PART_XML_FILEPATH)
                .getParts()
                .forEach(parDTO -> {
                    try {
                        if (!validation.isValid(parDTO)) {
                            throw new Exception("Invalid part: " + parDTO.getName() + "\n" +
