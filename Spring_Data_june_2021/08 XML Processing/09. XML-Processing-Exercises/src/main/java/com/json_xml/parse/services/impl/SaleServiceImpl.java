...
import com.json_xml.parse.models.dto.partCarSale.out.SalesRootOutDto;
...
public class SaleServiceImpl implements SaleService {
    ...
    public SaleServiceImpl(SaleRepository saleRepository, ModelMapper modelMapper,
                           XmlParser xmlParser, ValidationUtil validationUtil, IOUtil ioUtil, CarService carService, CustomerService customerService) {
...
//              amount of discount to be applied (discounts can be 0%, 5%, 10%, 15%, 20%, 30%, 40% or 50%).
    ...
    @Override
    public void getWithDiscounts() throws JAXBException {
        SalesRootOutDto salesRootOutDto = new SalesRootOutDto();

        salesRootOutDto.setSaleDtos(saleRepository.findAll()
                .stream().map(sale -> {
...
xmlParser.exportXML(salesRootOutDto, OUT_DIR_XML_FILEPATH+OUT_SALES_DISCOUNT_FILE);
